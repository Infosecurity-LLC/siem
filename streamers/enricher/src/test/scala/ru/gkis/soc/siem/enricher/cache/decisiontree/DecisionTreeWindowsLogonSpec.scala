...
class DecisionTreeWindowsLogonSpec extends FlatSpec with Matchers {
    ...
    private val rules: List[Rule[Int, Nothing]] = {
        ...
        val r10 = Rule[Int, Nothing](
            id = 10,
            tp = WindowsLogon,
            subj = Subject("org1", Some("someone.dp"), 0L, ScheduleGroup(Set(Schedule(5 * 60 * 60, 21 * 60 * 60, 5, 2, true))), None),
            obj = None,
            source = Some(Location(None, None)),
            destination = Some(Location(Some("someone"), Some("192.168.33.44"))),
            schedule = ScheduleGroup(Set(Schedule(5 * 60 * 60, 17 * 60 * 60, 5, 2, true))),
        ...
        val r11 = Rule[Int, Nothing](
            ...
            source = Some(Location(None, None)),
            destination = Some(Location(Some("someone"), Some("192.168.33.44"))),
            schedule = ScheduleGroup(Set(Schedule(5 * 60 * 60, 21 * 60 * 60, 0, 7, true))),
            aux1 = Some(Aux(Set(2))),
            aux2 = None,
            result = Allowed
        )
    ...
    tree.addAll(WindowsLogonControl[Int, Nothing](rules))

    private val treeShuffled = new DecisionTree[Int, Nothing]
    private val rulesShuffled = Random.shuffle(rules)
    treeShuffled.addAll(WindowsLogonControl[Int, Nothing](rulesShuffled))

    private val treeNoDefault = new DecisionTree[Int, Nothing]
    private val rulesNoDefault = rules.drop(1)
    treeNoDefault.addAll(WindowsLogonControl[Int, Nothing](rulesNoDefault))

    private def control(t: DecisionTree[Int, Nothing], evt: SocEvent): RuleResult = {
        WindowsLogonControl.decisionPath(evt) match {
            case Some(path) =>
                t.newControl(path, aux1 = Some(Lens.unit[SocEvent].interaction.logonType)).check(evt, ProductionCalendar(Set.empty))
            case None =>
                Undefined
        }
    }

    private def controlWithSchedule(t: DecisionTree[Int, Nothing], evt: SocEvent): RuleResult = {
        WindowsLogonControl.decisionPath(evt) match {
            case Some(path) =>
                t.newControl(
                    path,
                ...
                ).check(evt, ProductionCalendar(Set.empty))
            case None =>
                Undefined
        }
    }
    ...
    "Admin" should "be able to login into any computer by ip except bloomberg" in {
        val evt = SocEvent(
            collector = Some(CollectorInfo(organization = "org1")),
            subject = Some(SubjectInfo(name = Some("admin"), domain = Some("org1"))),
            source = Some(InteractingAssetLocation(ip = Some("admincomp"))),
    ...
    "User2" should "be able to login to a terminal at working hours" in {
        val evt = SocEvent(
            ...
            interaction = Some(InteractionDescription(logonType = Some(10))),
            eventTime = 1603290617L
        )
        controlWithSchedule(tree, evt) shouldBe Allowed
        controlWithSchedule(treeShuffled, evt) shouldBe Allowed
        controlWithSchedule(treeNoDefault, evt) shouldBe Allowed
    }
    ...
    "User2" should "not be able to login to a terminal after hours" in {
        val evt = SocEvent(
            ...
            destination = Some(InteractingAssetLocation(hostname = Some("work_terminal"))),
            interaction = Some(InteractionDescription(logonType = Some(10))),
            eventTime = 1603314000L
        )
        controlWithSchedule(tree, evt) shouldBe Restricted
        controlWithSchedule(treeShuffled, evt) shouldBe Restricted
        controlWithSchedule(treeNoDefault, evt) shouldBe Restricted
    }
    ...
    "Unknown organizations" must "be treated as Undefined" in {
        ...
        control(tree, evt) shouldBe Undefined
        control(treeShuffled, evt) shouldBe Undefined
        control(treeNoDefault, evt) shouldBe Undefined
    }
    ...
    "[SOC-1315] Logon control" should "be case insensitive" in {
        val evt = SocEvent(
            collector = Some(CollectorInfo(organization = "org1")),
            subject = Some(SubjectInfo(category = Counterpart.account, name = Some("someone.dp"), domain = Some("ISS"))),
            ...
            source = Some(InteractingAssetLocation(host = "192.168.66.77", ip = Some("192.168.66.77"), hostname = Some("SOMEONE"))),
            destination = Some(InteractingAssetLocation(host = "192.168.33.44", ip = Some("192.168.33.44"), hostname = Some("SOMEONE"))),
            data = Some(DataPayload(originTime = 1613735618))
        )
        control(tree, evt) shouldBe Allowed
        control(treeShuffled, evt) shouldBe Allowed
        control(treeNoDefault, evt) shouldBe Allowed
